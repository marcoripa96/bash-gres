...
- **Tables**: `fs_version_roots`, `fs_versions`, `version_ancestors`, `fs_entries`, `fs_blobs`, `fs_blob_chunks` (opt-in chunk-level index; FK-cascades with its blob; searched via `searchChunks()` when FTS is on, `searchChunksHybrid()` when vector search is too), `fs_chunk_embeddings` (vector-search only: per-content_hash embedding cache filled by `indexChunkEmbeddings()`; no FK — outlives its chunk rows by design)
