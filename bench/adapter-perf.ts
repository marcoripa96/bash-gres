 ...
 * node-postgres, Drizzle, Prisma) and prints a side-by-side comparison so
 * regressions in any single adapter — particularly the new Prisma one —
 * are obvious.
 *
 * Usage:
 *   docker compose up -d
 *   npm run prisma:generate     # if tests/prisma/generated is missing
 *   npm run bench:adapters
 *
 * Optional env:
 *   TEST_DATABASE_URL  override connection string (default: localhost:5433)
 *   BENCH_ADAPTERS     comma-separated subset, e.g. "postgres,prisma"
...
import { createPrismaClient, type PrismaLike } from "../lib/adapters/prisma/adapter.js";
...
// Loaded dynamically so the bench compiles cleanly without depending on the
// generated Prisma client tree (which lives outside `dist-bench/` after
// `tsc -p tsconfig.bench.json`).
async function loadPrismaClient(): Promise<new (opts?: unknown) => unknown> {
  const generated = path.resolve(
    process.cwd(),
    "tests/prisma/generated/client.js",
  );
  const mod = (await import(generated)) as {
...
async function makePrismaAdapter(): Promise<AdapterHandle> {
  const Ctor = await loadPrismaClient();
  const prisma = new Ctor({
    datasources: { db: { url: TEST_DB_URL } },
  }) as PrismaLike & { $disconnect(): Promise<void> };
  return {
    name: "prisma",
    client: createPrismaClient(prisma),
